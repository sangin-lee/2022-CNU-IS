...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    result = bitarray()
    for i in range(0, count):
        temp = prev_result ^ text[i*8 : (i+1)*8]
        prev_result = sdes(temp, key, MODE_ENCRYPT)
        result += prev_result
    return result
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    result = bitarray()
    for i in range(0, count):
        temp = sdes(ciphertext[i*8 : (i+1)*8], key, MODE_DECRYPT)
        result += prev_text ^ temp
        prev_text = ciphertext[i*8 : (i+1)*8]
    return result
...
random_iv = bitarray()
for i in range(8):
    random_iv.append(random.randint(0, 1))
